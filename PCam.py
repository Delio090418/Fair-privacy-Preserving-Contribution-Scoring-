...
x_path = data_paths["pcam"][0]
y_path = data_paths["pcam"][1]
...
def split_indices_train_test(indices, y_data, test_size=0.2, seed=42):
    ...
    labels = np.array(y_data[np.sort(indices)]).flatten()  
...
def non_iid_data_pcam(x_path=x_path, y_path=y_path, n_clients=6, alpha=0.5, seed=42):
