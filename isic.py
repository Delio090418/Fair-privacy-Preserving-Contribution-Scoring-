...
image_dir = data_paths["isic"][0]
csv_path = data_paths["isic"][1]
...
def non_iid_dirichlet_split(df, n_clients=6, alpha=0.1, seed=42):
    np.random.seed(seed)
...
class ISIC2019Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label = int(row['label_id'])  # Ensure it's an int
        if self.transform:
...
def non_iid_data(n_clients=6, alpha=0.1, seed=42):
    ...
    for i in range(n_clients):
        train_loader = DataLoader(clients_train_data[i], batch_size=BATCH_SIZE, shuffle=SHUFFLE,num_workers=0,
    pin_memory=True)
        test_loader = DataLoader(clients_test_data[i], batch_size=BATCH_SIZE, shuffle=False,num_workers=0,
    pin_memory=True)
...
def commun_test_isic(commun):
        ...
        combined_dataloader = DataLoader(combined, batch_size=32, shuffle=False,num_workers=0,
    pin_memory=True)
