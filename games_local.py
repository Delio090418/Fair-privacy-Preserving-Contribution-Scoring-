...
from modelos import CNN_brain

dev = "mps" if torch.backends.mps.is_available() else "cpu"
dev = torch.device(dev)

class games(federation):
    def __init__(self, model_type, dataset_name, num_clients,partition_type,alpha):
        super().__init__(model_type, dataset_name, num_clients,partition_type,alpha)
    ...
    def retrainin_game(self,iterations, con_clientes, epochs=5):
        ...
        worth = self.evaluation_global(self.test_global, device=dev)
        return worth
    ...
    def local_coalicional_value_at_round(self, iter, path, epochs=5):
        ...
        for subset_len in range(2,self.num_clients+1):
            for subset_idx in combinations(list(range(self.num_clients)), subset_len):
                ...
                params = self.weighted_aggre(part,dev)
                self.model.to(dev)  # Ensure the model is on the correct device
                self.set_parameters(params)

                # Select GPU in round-robin manner

                #tqdm.write(f"Evaluating subset {subset_idx} on {selected_gpu}")
                d[subset_idx] = self.evaluation_global(self.test_global, device=dev)
        ...
        #         subset_indices = list(combinations(list(range(self.num_clients)), subset_len))
        #         for idx, subset_idx in enumerate(subset_indices):
        #             part = [self.lista_clientes[i] for i in subset_idx]
        #             params = self.weighted_aggre(part)
        #             device = devices[idx % len(devices)]
        #             futures.append(executor.submit(self.evaluate_global_on_device, params, self.test_global, device))
        #         for idx, subset_idx in enumerate(subset_indices):
        #             d[subset_idx] = futures[idx].result()
        #     tqdm.write(f"subset size {subset_len}")
        # logger_f(f"at coalitional values: {d}", f"{path}/values/results_global.log")
    ...
    def valores_por_rondas(self,itera,path,epochs=5):   
            ...
            return base_metr,priv_pr,cosine#efecto
...
#this function has been modify to only consider the cos similarity. the original statistic function is test.py
    ...
    def statistic(self,path,iter_one,iter_two=10,epochs=5):
        ...
        for i in range(iter_one):
            ...
            #matrix
            for_mean_glo=[valores[0]]+valores[1]+[valores[2]]
            matrix_scores_last_rounds_gl.append(np.array(for_mean_glo).T)
        ...
        return arrays_lr,valores,metrica,np.array(matrix_scores_last_rounds_gl)#,np.array(matrix_scores_last_rounds_lo)#,#,metrmean,effects_EE
    ...
    def simulation_global_iter(self,iteraci=10,iter_fed=10,epochs=1):
            ...
            self.boxplot(transform_re[0],metricas_global,path1,"last_round",["I1I","L1O","IE2I","LE2O","SE","EE","PPCE","f_I1I","f_L1O", "COS"])
    ...
    def simulations_retraining(self,iteraci=10,iter_fed=10,epochs=1):
            if self.partition_type=="N-IID":
                path1=f'./RESULTS_local/{self.dataset_name}/{self.num_clients}cli({self.alpha})_{iter_fed}fed_retrain'
                title=f"Case N-IID, alpha={self.alpha}, and iter={iter_fed}"
                os.makedirs(path1, exist_ok=True)
            elif self.partition_type=='IID':
                path1=f'./RESULTS_local/{self.dataset_name}/{self.num_clients}cli({self.partition_type})_{iter_fed}fed_retrain'
                title=f"Case {self.partition_type}, and iter={iter_fed}"
                os.makedirs(path1, exist_ok=True)
        ...
        if retrining==None:
            ...
            if par_type=="N-IID":
                ...
                for i in range(4):
                    ...
                    logger_f(f"{data_name}, num_cli={num_cli}, alpha={alpha}, iter_global={iter_global}, iter_fed={iter_fed}, {valores[i]}",path)
            elif par_type=="IID":
                ...
                for i in range(4):
                    ...
                    logger_f(f"{data_name}, num_cli={num_cli}, {par_type}, iter_global={iter_global}, iter_fed={iter_fed}, {valores[i]}",path)
        elif retrining=="ON":
...
if __name__ == "__main__":
    ...
    dict_exper={
       0:["CNN","cifar10",3,"IID",0.5,1,0,1,"ON"], #This is for alpha = 0.1
    ...
    # dict_exper={
    #             #0:["resnetpcam","pcam",3,"N-IID",0.5,2,1,5,"ON"],#script
    #             #0:["resnet","isic",3,"N-IID",0.5,2,1,3,"ON"],#script_two
    #             #0:["CNN","cifar10",3,"N-IID",0.5,2,1,5,"ON"],#script_three
    #             0:["CNN_brain","BRAIN",3,"N-IID",0.5,2,1,5,"ON"],#script_four
